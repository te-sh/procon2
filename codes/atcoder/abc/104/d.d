// URL: https://atcoder.jp/contests/abc104/tasks/abc104_d
...
void main()
{
  ...
  auto n = s.length.to!int, q = 0;
  auto a = new mint[](n+1), b = new mint[](n+1), c = new mint[](n+1);
  foreach (i, si; s)
    switch (si) {
    case 'A':
      a[i+1] = a[i]+mint(3).repeatedSquare(q);
      b[i+1] = b[i];
      c[i+1] = c[i];
      break;
    case 'B':
    ...
    case 'C':
      ...
      break;
    case '?':
      a[i+1] = a[i]*3+mint(3).repeatedSquare(q);
      b[i+1] = b[i]*3+a[i];
      c[i+1] = c[i]*3+b[i];
      ++q;
      break;
