...
void main()
{
  int n, x, y, z; readV(n, x, y, z);

  auto k = x+y+z-1, dp = new mint[][](n+1, 1<<k);
  dp[0][0] = 1;

  foreach (i; 0..n)
    foreach (j; 0..1<<k)
      foreach (d; 1..11) {
        auto m = (j<<d).bitSet(d-1);
        if (!(m.bitTest(z-1) && m.bitTest(z+y-1) && m.bitTest(z+y+x-1)))
          dp[i+1][m&((1<<k)-1)] += dp[i][j];
      }

  writeln(mint(10).repeatedSquare(n) - dp[n].sum);
}
