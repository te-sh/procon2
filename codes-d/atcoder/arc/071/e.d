...
void main()
{
  ...
  auto sf = s.map!(si => si == 'A' ? 1 : 2).array;
  auto tf = t.map!(ti => ti == 'A' ? 1 : 2).array;

  auto sc = sf.cumulativeSum;
  auto tc = tf.cumulativeSum;

  int q; readV(q);
  foreach (_; 0..q) {
    int a, b, c, d; readV(a, b, c, d); --a; --c;
    writeln(sc[a..b]%3 == tc[c..d]%3 ? "YES" : "NO");
  }
}
