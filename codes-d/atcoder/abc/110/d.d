...
void main()
{
  ...
  auto q = f.values, mq = q.reduce!max;

  auto fact = new mint[](mq+n); fact[0] = 1;
  foreach (i; 1..mq+n) fact[i] = fact[i-1]*i;
  auto invFact = new mint[](mq+n); invFact[$-1] = fact[$-1].inv;
  foreach_reverse (i; 1..mq+n) invFact[i-1] = invFact[i]*i;

  auto r = mint(1);
  foreach (qi; q)
    r *= fact[qi+n-1]*invFact[n-1]*invFact[qi];
