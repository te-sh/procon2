...
struct Vector(T)
{
   ...
   ** zero は加法単位元です.
   **/
  this(size_t n, T zero = T(0)) in { assert(n > 0); } do
  {
   ...
   ** zero は加法単位元です.
   **/
  this(U)(U[] b, T zero = T(0)) in { assert(b.length > 0); } do
  {
   ...
   ** v と b の次元は同じである必要があります.
   **/
  ref Vector!T opOpAssign(string op)(Vector!T b) if (op=="+"||op=="-")
  in { assert(n == b.n); } do
   ...
   ** v*=b を計算します. b は数値です.
   **/
  ref Vector!T opOpAssign(string op: "*")(T b)
 ...
 ** ベクトル a 同士の内積を返します.
 **/
pure T hypot2(T)(Vector!T a) { return a*a; }
/**
 ** ベクトル a, b のクロス積を返します.
 ** a, b の次元は 3 である必要があります.
 **/
pure Vector!T cross(T)(Vector!T a, Vector!T b) in { assert(a.n == 3 && b.n == 3); } do
...
struct Matrix(T)
{
   ...
   ** r 行 c 列の空行列を返します.
   **/
  this(size_t r, size_t c, T zero = T(0), T one = T(1)) in { assert(r > 0 && c > 0); } do
  {
   ...
   ** m と b の行数, 列数は同じである必要があります.
   **/
  ref Matrix!T opOpAssign(string op)(Matrix!T b) if (op=="+"||op=="-")
  in { assert(r == b.r && c == b.c); } do
  {
   ...
   ** m*=b を計算します. b は数値です.
   **/
  ref Matrix!T opOpAssign(string op: "*")(T b)
  {
   ...
   ** m の列数と b の行数は同じである必要があります.
   **/
  ref Matrix!T opOpAssign(string op: "*")(Matrix!T b) in { assert(c == b.r); } do
  {
    auto x = this*b;
    r = x.r; c = x.c; a = x.a;
    return this;
 ...
 ** a は正方行列である必要があります.
 **/
pure T det(T)(Matrix!T a) in { assert(a.r == a.c); } do
{
